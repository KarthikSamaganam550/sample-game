...
let writeStream = fs.createWriteStream('metadata.log');


server.listen(port, function () {
  console.log('Server listening at port %d', port);

  writeStream.write('started') 
});
...
function buildGame(socket) {
 ...
 writeStream.write("\n Game Created by "+ socket.username + " w/ " + gameObject.id);
...
function killGame(socket) {
  ...
  for(var i = 0; i < gameCollection.totalGameCount; i++){
    ...
    if (plyr1Tmp == socket.username){
      --gameCollection.totalGameCount; 
      console.log("\n Destroy Game "+ gameId + "!");
      writeStream.write("Destroy Game "+ gameId + "!");

      gameCollection.gameList.splice(i, 1);
      console.log(gameCollection.gameList);
      writeStream.write(gameCollection.gameList)
      ...
      writeStream.write("\n" +socket.username + ": has left " + gameId)
      socket.emit('leftGame', { gameId: gameId });
...
function gameSeeker (socket) {
  ...
  if (( gameCollection.totalGameCount == 0) || (loopLimit >= 20)) {
    ...
    if (gameCollection.gameList[rndPick]['gameObject']['playerTwo'] == null)
    {
      ...
      writeStream.write("\n"+socket.username + ": has been added to " + gameCollection.gameList[rndPick]['gameObject']['id'])
    } else {
